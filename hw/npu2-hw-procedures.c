...
static uint32_t phy_tx_zcal_calculate(struct npu2_dev *ndev)
{
	...
	if (ndev->type == NPU2_DEV_TYPE_OPENCAPI &&
	    platform.ocapi->phy_tune) {
		ffe_pre_coeff = platform.ocapi->phy_tune->tx_ffe_pre_coeff;
		ffe_post_coeff = platform.ocapi->phy_tune->tx_ffe_post_coeff;
	}
...
void npu2_opencapi_phy_init(struct npu2_dev *dev)
{
	...
	if (platform.ocapi->phy_tune) {
		phy_write(dev, &NPU2_PHY_TX_FFE_BOOST_EN,
			  platform.ocapi->phy_tune->tx_ffe_boost_en);
	}
	// fxb
	val = phy_read(dev, &NPU2_PHY_TX_FFE_BOOST_EN);
	OCAPIINF(dev, "ffe_boost_en=%llx\n", val);
	// fxb
